...
if [ -f "$FLINK_CONF_DIR/flink_env.sh" ]; then source "$FLINK_CONF_DIR/flink_env.sh"; fi
...
TMWorkers() {
    ...
    if [ ${WORKERS_ALL_LOCALHOST} = true ] ; then
        ...
        if [[ $? -ne 0 ]]; then
            echo "Starting taskmanagers through ssh ${WORKERS[@]}"
            for worker in ${WORKERS[@]}; do
                ssh -n $FLINK_SSH_OPTS $worker -- "nohup /bin/bash -l \"${FLINK_BIN_DIR}/taskmanager.sh\" \"${CMD}\" &"
            done
        else
            echo "Starting taskmanagers through pdsh ${WORKERS[@]}"
